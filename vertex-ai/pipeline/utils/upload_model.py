...
def upload_model(
    ...
    comparable: bool,
    pipeline_job_id: str,
    serving_container_image: str = None,
    model_evaluation: Input[Metrics]=None,
    eval_metric: str=None,
    eval_lower_is_better: bool=None,
    ...
    def lookup_model(model_name: str) -> aip.Model:
        ...
        if len(models) == 0:
            logging.info(f"No model found with name {model_name}" + f"(project: {project} location: {location})")
            return None
        elif len(models) == 1:
            return models[0]
        else:
    ...
    def upload_model_to_registry(is_default_version: bool, parent_model_uri: str = None) -> Model:
        ...
        vertex_model.uri = (
            f"https://{location}-aiplatform.googleapis.com/v1/" f"{uploaded_model.versioned_resource_name}"
        )
    ...
    def import_evaluation(
        ...
        client = ModelServiceClient(client_options={"api_endpoint": location + "-aiplatform.googleapis.com"})
    ...
    if champion_model is None or not comparable:
